...
export default function TeachersTab({
  ...
  const [manageAssignScheduleIds, setManageAssignScheduleIds] = useState<
    number[]
  >([]);
  const [manageScheduleSearchQuery, setManageScheduleSearchQuery] =
    useState("");
    ...
    if (response.success && response.teacherId) {
      if (selectedScheduleIds.length > 0) {
        ...
        if (!schedResponse.success) {
          setIsError(true);
          setMessage(
            `Staff created, but class batch assignment failed: ${schedResponse.message}`,
          );
      ...
      setMessage(
        selectedScheduleIds.length > 0
          ? `Staff account created and ${selectedScheduleIds.length} classes assigned.`
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {paginatedTeachers.map((teacher) => {
          ...
          return (
            <div
              key={teacher.id}
              className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-[#011B51]/30 transition-all flex flex-col justify-between h-full"
            >
              <div className="flex items-start gap-4 mb-6">
                ...
                <div className="flex flex-col pt-0.5">
                  ...
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono mt-1">
                    ID: {teacher.user_id}
                  </span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                <div
                  className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md ${assignedCount > 0 ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden animate-in zoom-in-95 duration-200 border-t-8 border-[#FED702] flex flex-col max-h-[90vh]">
            ...
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 flex-shrink-0 gap-4">
              <div>
                ...
                <h2 className="text-2xl font-black text-[#011B51] uppercase tracking-tight">
                  Register Instructor
                </h2>
              </div>
              <button
                onClick={closeRegisterModal}
                className="text-slate-400 hover:text-[#011B51] font-black text-3xl cursor-pointer transition-colors leading-none"
              >
                &times;
              </button>
            </div>
            ...
            <div className="p-8 overflow-y-auto flex-1 bg-white">
              {message && (
                <div
                  className={`p-4 rounded-xl text-xs font-bold uppercase tracking-wide text-center mb-8 border-2 ${isError ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}
                >
                  {message}
                </div>
              )}
              ...
              >
                ...
                <div className="flex flex-col h-full space-y-5">
                  <label className="block text-sm font-black text-[#011B51] uppercase tracking-widest ml-1">
                    Account Details
                  </label>

                  <div>
                    <label className="block text-[10px] sm:text-xs font-bold text-[#011B51] uppercase tracking-wide mb-1.5 ml-1">
                      Instructor ID
                    </label>
                    <input
                      type="text"
                ...
                <div className="flex flex-col h-full border-t pt-8 md:pt-0 md:border-t-0 md:border-l md:pl-10 border-slate-200">
                  <label className="block text-sm font-black text-[#011B51] uppercase tracking-widest mb-4 ml-1">
                  ...
                  <div className="flex flex-col gap-4 flex-1">
                    ...
                    <div className="border-2 border-slate-200 rounded-xl overflow-hidden bg-white flex flex-col flex-1 shadow-sm">
                      <div className="px-5 py-3 bg-slate-100 border-b border-slate-200 flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                        ...
                        <span className="bg-[#011B51] text-white px-2 py-0.5 rounded text-[9px]">
                      ...
                      <ul className="max-h-72 overflow-y-auto p-3 space-y-2">
                        {filteredSchedules.length === 0 ? (
                          ...
                          filteredSchedules.map((sched: any) => (
                            <li key={sched.id}>
                              <label
                                className={`flex items-start space-x-4 p-3.5 rounded-xl cursor-pointer transition-all ${selectedScheduleIds.includes(sched.id) ? "bg-[#011B51]/5 border border-[#011B51]/30 shadow-sm" : "bg-slate-50 hover:bg-slate-100 border border-slate-100"}`}
                              >
                                ...
                                <div className="flex-1">
                                  <div className="flex justify-between items-start">
                                    ...
                                    <span className="text-[9px] font-black text-[#011B51] bg-[#011B51]/10 px-2.5 py-0.5 rounded uppercase tracking-widest">
                                      {sched.date}
                                    </span>
                                  </div>
                                  <div className="text-[11px] font-bold text-slate-500 mt-1.5 uppercase tracking-widest">
                                    Sec {sched.section} | {sched.lab_room}
                                  </div>
                                </div>
                              </label>
                            </li>
            ...
            <div className="px-8 py-5 border-t border-slate-200 bg-slate-50 flex justify-end gap-4 flex-shrink-0">
              ...
              >
                {isProcessing
                  ? "Processing..."
                  : selectedScheduleIds.length > 0
                    ? `Create Account & Assign ${selectedScheduleIds.length} Classes`
                    : "Create Account (No Classes)"}
              </button>
            </div>
          </div>
        </div>
            ...
            <div className="px-8 py-6 border-b border-slate-100 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-slate-50 flex-shrink-0 gap-4">
              <div>
                ...
                <div className="flex items-end gap-3">
                  ...
                  <span className="mb-1 text-xs font-bold text-[#011B51] bg-[#011B51]/10 px-2 py-1 rounded-md uppercase tracking-widest">
                    ID: {manageTeacher.user_id}
                  </span>
                </div>
              </div>
                ...
                <div className="flex flex-col h-full">
                  ...
                  <ul className="space-y-3 overflow-y-auto pr-2 flex-1 max-h-96">
                    ...
                    {teacherSchedules.map((sched) => (
                      <li
                        key={sched.id}
                        className="flex justify-between items-center bg-white border-2 border-slate-100 p-4 rounded-xl transition-all hover:border-slate-300 shadow-sm hover:shadow-md"
                      >
                        <div>
                ...
                <div className="flex flex-col h-full border-t pt-8 md:pt-0 md:border-t-0 md:border-l md:pl-10 border-slate-200">
                  ...
                  <div className="flex flex-col gap-4 flex-1">
                    <input
                      ...
                      onChange={(e) =>
                        setManageScheduleSearchQuery(e.target.value)
                      }
                    />
                    ...
                    <div className="border-2 border-slate-200 rounded-xl overflow-hidden bg-white flex flex-col flex-1 shadow-sm">
                      <div className="px-5 py-3 bg-slate-100 border-b border-slate-200 flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                        <span>Database Inventory</span>
                        <span className="bg-[#011B51] text-white px-2 py-0.5 rounded text-[9px]">
                          {manageAssignScheduleIds.length} Selected
                        </span>
                      </div>
                      ...
                      <ul className="max-h-72 overflow-y-auto p-3 space-y-2">
                        {filteredAvailableSchedules.length === 0 ? (
                          ...
                          filteredAvailableSchedules.map((sched: any) => (
                            <li key={sched.id}>
                              <label
                                className={`flex items-start space-x-4 p-3.5 rounded-xl cursor-pointer transition-all ${manageAssignScheduleIds.includes(sched.id) ? "bg-[#011B51]/5 border border-[#011B51]/30 shadow-sm" : "bg-slate-50 hover:bg-slate-100 border border-slate-100"}`}
                              >
                                <div className="relative flex items-center justify-center mt-0.5">
                                  <input
                                    type="checkbox"
                                    checked={manageAssignScheduleIds.includes(
                                      sched.id,
                                    )}
                                    onChange={() =>
                                      handleToggleManageSchedule(sched.id)
                                    }
                                    className="peer appearance-none w-5 h-5 border-2 border-slate-300 rounded flex-shrink-0 checked:bg-[#011B51] checked:border-[#011B51] transition-all cursor-pointer shadow-sm"
                                  />
                                  <svg
                    ...
                    <button
                      ...
                      disabled={
                        isManageProcessing ||
                        manageAssignScheduleIds.length === 0
                      }
                    ...
                    >
                      {isManageProcessing
                        ? "Assigning..."
                        : manageAssignScheduleIds.length > 0
                          ? `Link ${manageAssignScheduleIds.length} Selected Classes`
                          : "Select Classes to Link"}
                    </button>
                  </div>
                </div>
              </div>
